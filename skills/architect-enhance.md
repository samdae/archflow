...
projectRoot/
  └── docs/
        └── {serviceName}/
              ...
              ├── requirements-v2.md     # ← Enhancement requirements (require-refine output)
              ├── architect.md           # ← Existing design (synced state recommended)
              └── changelog.md
```
...
- `is_synced: no` → Recommend running **architect-sync** first
...
```
[require-refine] → existing requirements.md

(Enhancement needed)
        ↓
[require-refine] → requirements-v2.md (enhancement requirements)
        ↓
[architect-enhance] ← synced architect.md
        ↓
Updated architect.md
        ↓
[implement] → Enhancement implementation
```
